...
def send_reply_with_attachment_fixed(to_addr, subject, excel_file_path, original_subject, email_type='import'):
    """发送回复邮件并附加Excel文件，支持额外收件人"""
    try:
        # 检查文件是否存在
        ...
        # 获取配置的额外收件人（从config_manager）
        additional_recipients = []
        try:
            # 这里假设config_manager有get_additional_recipients方法
            # 如果没有，需要在config_manager.py中添加该方法
            if hasattr(config_manager, 'get_additional_recipients'):
                additional_recipients = config_manager.get_additional_recipients(email_type)
            else:
                # 临时方案：从配置文件中读取额外收件人
                if email_type in config.get('email', {}):
                    additional_recipients_str = config['email'].get(f'{email_type}_additional_recipients', '')
                    if additional_recipients_str:
                        additional_recipients = [addr.strip() for addr in additional_recipients_str.split(',') if addr.strip()]
        except Exception as e:
            logging.warning(f"⚠️ 获取额外收件人失败: {e}")
            ...
            for txt_attachment in txt_attachments:
                ...
                if container_data:
                    ...
                    if create_excel_file(container_data, excel_filename):
                        # 发送回复邮件
                        if send_reply_with_attachment_fixed(from_header, subject, excel_filename, subject, 'import'):
                            excel_sent = 1
